...
class LiveStreamViewController: UIViewController {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell{
        ...
        if indexPath.row == 0 {
            cell.imageView!.image = UIImage(named: "Live Stream")
            cell.textLabel?.text = "Live Stream 1"
            cell.layer.cornerRadius = 1.0
            
        } else if indexPath.row == 1 {
            cell.imageView!.image = UIImage(named: "Live Stream")
            cell.textLabel!.text = "Live Stream 2"
            cell.layer.cornerRadius = 1.0
        }
        else if indexPath.row == 2 {
            cell.imageView!.image = UIImage(named: "Live Stream")
            cell.textLabel!.text = "Live Stream 3"
            cell.layer.cornerRadius = 1.0
        }
        else if indexPath.row == 3 {
            cell.imageView!.image = UIImage(named: "Live Stream")
            cell.textLabel!.text = "Live Stream 4"
            cell.layer.cornerRadius = 1.0
        }else {
            cell.imageView!.image = UIImage(named: "Live Stream")
            cell.textLabel!.text = "Live Stream 5"
            cell.layer.cornerRadius = 1.0
        }
        return cell
    }
